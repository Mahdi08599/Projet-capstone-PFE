...
Le script charge les probabilites finales du modele XGBoost optimise, reconstruit le meme echantillon de validation stratifie, puis compare trois groupes : les fraudes detectees, les fraudes ratees et les transactions legitimes. Pour cette analyse d'erreur, un seuil d'etude de \textbf{0,40} a ete utilise afin d'observer les fraudes qui resteraient non signalees avec un seuil plus permissif que le seuil final de decision. Sur l'ensemble de validation, \textbf{536 fraudes sur 4 133} obtiennent un score inferieur a 0,40. Elles constituent donc le coeur de l'analyse des faux negatifs.

\begin{table}[H]
\centering
\caption{Synthese de l'analyse des fraudes non detectees au seuil 0,40}
\begin{tabular}{lc}
\toprule
\textbf{Indicateur} & \textbf{Valeur} \\
\midrule
Fraudes totales dans l'ensemble de validation & 4 133 \\
Fraudes non detectees avec score $< 0,40$ & 536 \\
Part des fraudes non detectees & 13,0\% \\
Fraudes proches du seuil ($0,30 \leq score < 0,40$) & 169 \\
Fraudes avec score tres bas ($score < 0,10$) & 61 \\
Montant median des fraudes ratees & 87 USD \\
Montant median des fraudes detectees & 73 USD \\
\bottomrule
\end{tabular}
\end{table}
...
Le script \texttt{src/analyze\_missed\_frauds.py} complete l'evaluation du modele final en analysant les faux negatifs. Il charge les predictions finales sauvegardees dans \texttt{reports/results/final\_predictions.npz}, reconstruit l'ensemble de validation avec le meme split stratifie, puis identifie les fraudes dont le score est inferieur a 0,40.
