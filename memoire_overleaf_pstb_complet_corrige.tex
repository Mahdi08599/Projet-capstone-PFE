...
% THEME PST&B
% =========================================================
\definecolor{PSTBPurple}{RGB}{74,35,118}
\definecolor{PSTBViolet}{RGB}{112,57,176}
\definecolor{PSTBDeep}{RGB}{32,21,55}
\definecolor{PSTBAccent}{RGB}{187,80,210}
\definecolor{PSTBLight}{RGB}{245,240,250}
\definecolor{PSTBInk}{RGB}{34,33,44}
\definecolor{linkblue}{RGB}{58,41,145}

\hypersetup{
    colorlinks=true,
    linkcolor=PSTBPurple,
    urlcolor=PSTBViolet,
    citecolor=linkblue
}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\textcolor{PSTBDeep}{\leftmark}}
\fancyhead[R]{\small\textcolor{PSTBPurple}{PST\&B -- Mastère Data Science in Business}}
\fancyfoot[C]{\textcolor{PSTBPurple}{\thepage}}
\renewcommand{\headrulewidth}{0.4pt}
\renewcommand{\headrule}{\hbox to\headwidth{\color{PSTBViolet}\leaders\hrule height \headrulewidth\hfill}}

\titleformat{\chapter}[display]
{\normalfont\bfseries\Huge\color{PSTBPurple}}
{\chaptertitlename\ \thechapter}{20pt}{\Huge}

\titleformat{\section}
{\normalfont\Large\bfseries\color{PSTBPurple}}
{\thesection}{1em}{}

\titleformat{\subsection}
{\normalfont\large\bfseries\color{PSTBViolet}}
{\thesubsection}{1em}{}
...
\lstset{
    ...
    breaklines=true,
    frame=single,
    backgroundcolor=\color{PSTBLight},
    keywordstyle=\color{PSTBPurple}\bfseries,
    commentstyle=\color{PSTBViolet},
    stringstyle=\color{PSTBAccent},
    numbers=left,
...
% Logo PST&B robuste : Overleaf affiche le logo si un de ces fichiers existe,
% sinon un cartouche texte PST&B est utilisé.
\newcommand{\PSTBLogo}[1][1.25cm]{%
    \IfFileExists{figures/pstb_logo.jpg}{\includegraphics[height=#1,keepaspectratio]{figures/pstb_logo.jpg}}{%
        \IfFileExists{pstb_logo.jpg}{\includegraphics[height=#1,keepaspectratio]{pstb_logo.jpg}}{%
            \IfFileExists{figures/pstb_logo.png}{\includegraphics[height=#1,keepaspectratio]{figures/pstb_logo.png}}{%
                \IfFileExists{pstb_logo.png}{\includegraphics[height=#1,keepaspectratio]{pstb_logo.png}}{%
                    \fcolorbox{white}{white}{\textcolor{PSTBPurple}{\Large\bfseries PST\&B}}%
                }%
            }%
        }%
    }%
...
\begin{tikzpicture}[remember picture, overlay]
    % Fond principal violet PST&B
    \fill[PSTBDeep] (current page.north west) rectangle (current page.south east);
    \fill[top color=PSTBPurple, bottom color=PSTBDeep]
        (current page.north west) rectangle ([yshift=-11cm]current page.north east);

    % Formes decoratives
    \fill[PSTBViolet, opacity=0.55]
        ([xshift=-2cm,yshift=-2cm]current page.north east) circle (4.2cm);
    \fill[PSTBAccent, opacity=0.35]
        ([xshift=1.5cm,yshift=-8.5cm]current page.north west) circle (3.8cm);
    \fill[white, opacity=0.06]
        (current page.north east) -- +(-7,0) -- +(0,-7) -- cycle;
    \fill[white, opacity=0.04]
        (current page.south west) -- +(8,0) -- +(0,8) -- cycle;

    % Bande blanche centrale
    \fill[white]
        ([xshift=1.45cm,yshift=-11.1cm]current page.north west)
        rectangle
        ([xshift=-1.45cm,yshift=2.4cm]current page.south east);

    % Logo PST&B : la macro accepte jpg/png, à la racine ou dans figures/
    \node[anchor=north west] at ([xshift=1.85cm,yshift=-1.25cm]current page.north west) {
        \PSTBLogo[1.45cm]
    };

    % Ecole et programme
    \node[anchor=north east, text=white, align=right, font=\large\bfseries]
        at ([xshift=-1.85cm,yshift=-1.35cm]current page.north east)
        {Paris School of Technology \& Business};
    \node[anchor=north east, text=white!85, align=right, font=\normalsize]
        at ([xshift=-1.85cm,yshift=-2.15cm]current page.north east)
    ...
    \node[anchor=center, text=white, font=\Huge\bfseries,
        ...
        {Détection de fraude dans\\[3pt] la banque digitale};

    \node[anchor=center, text=PSTBLight, font=\Large\bfseries,
          text width=14cm, align=center]
        at ([yshift=-7.35cm]current page.north)
        {Approche hybride Self-Supervised Learning \\[3pt] et XGBoost optimisé};

    % Badge capstone
    \node[anchor=center, text=PSTBPurple, font=\large\bfseries,
          fill=white, rounded corners=5pt, inner sep=10pt,
          drop shadow={shadow xshift=1pt, shadow yshift=-1pt, opacity=0.18}]
        at ([yshift=-9.65cm]current page.north)
        {Professional Capstone -- Mémoire de fin d'année};

    % Bloc informations
    \node[anchor=north, text=PSTBInk, font=\normalsize,
          text width=14cm, align=left]
        at ([yshift=-12.1cm]current page.north) {
        \begin{tabularx}{14cm}{>{\bfseries\color{PSTBPurple}}l X}
        Étudiants : & Mahdi BEN ARFI \quad \& \quad Mohamed EL HAMDAOUI \\[6pt]
        Encadrant : & Mahdi ZARG AYOUNA \\[6pt]
        École : & PST\&B -- Paris School of Technology \& Business \\[6pt]
        Formation : & Mastère Data Science in Business \\[6pt]
        Année universitaire : & 2025 -- 2026 \\[6pt]
        Projet : & Système d'aide à la décision pour la détection de fraude bancaire digitale \\
        \end{tabularx}
    };

    % Footer violet
    \fill[PSTBPurple]
        (current page.south west) rectangle ([yshift=1.7cm]current page.south east);
    \fill[PSTBAccent]
        ([yshift=1.7cm]current page.south west) rectangle ([yshift=2.05cm]current page.south east);
    \node[anchor=center, text=white!85, font=\small]
        at ([yshift=0.85cm]current page.south)
        {Version V1 synchronisée Overleaf -- Juin 2026};
\end{tikzpicture}
\end{titlepage}
\clearpage
\pagenumbering{roman}
...
\textbf{Mots clés :} fraude bancaire, banque digitale, XGBoost, self-supervised learning, KPIs métier.
...
Precision & Part des alertes du modèle qui correspondent réellement à des fraudes. \\
...
XGBoost is particularly suited to tabular data. It combines gradient boosting, regularization and efficient tree construction. It also allows class imbalance handling through the \texttt{scale\_pos\_weight} parameter, which increases the relative importance of fraud examples during training. In this project, XGBoost progressively became the final supervised model because it outperformed the neural SSL-based classifier on validation metrics.
...
\midrule
train\_transaction.csv & Donnees transactionnelles d'entrainement & 590 540 lignes, contient le label \texttt{isFraud}. \\
...
La phase Extract consiste a charger les fichiers CSV et a joindre les tables transaction et identity via la cle \texttt{TransactionID}. La phase Transform applique le nettoyage, l'imputation, l'encodage et la creation de features. La phase Load sauvegarde les donnees nettoyees en format Parquet, plus leger et plus rapide a charger que les CSV bruts.

\section{Preprocessing V1}

La premiere version du preprocessing visait a obtenir rapidement un dataset exploitable. Elle supprimait les colonnes contenant plus de 70\% de valeurs manquantes, imputait les valeurs manquantes numeriques par la mediane, remplacait les categories manquantes par \texttt{UNKNOWN}, encodait les variables categorielles et normalisait les variables numeriques.
...
La deuxieme version du preprocessing corrige ces limites. Le seuil de suppression des colonnes a ete releve a 95\%, ce qui permet de conserver beaucoup plus de variables. Des indicateurs de valeurs manquantes ont ete ajoutes afin que le modele puisse exploiter le fait qu'une information soit absente. Des features temporelles ont ete creees a partir de \texttt{TransactionDT}, notamment l'heure de transaction et les periodes de pic de fraude.
...
Le self-supervised learning a ete teste a travers une logique de reconstruction de features masquees. Pour chaque transaction, une partie des variables est masquees aleatoirement. Le modele doit ensuite reconstruire les valeurs cachees a partir des autres variables. Cette tache ne necessite pas le label \texttt{isFraud}, ce qui correspond au principe du SSL.
...
\begin{table}[H]
\centering
\caption{Comparaison avant et apres optimisation (chaque modele evalue a son propre seuil de meilleur F1 : 0,83 avant, 0,56 apres)}
\begin{tabular}{lccc}
\toprule
...
F1-score & 0,6644 & 0,8210 & +0,1566 \\
Precision & 0,7458 & 0,8797 & +0,1339 \\
Recall & 0,5991 & 0,7697 & +0,1706 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/final_roc.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/final_threshold_analysis.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/final_business_impact.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/business_produits_exposition.png}
...
Une analyse specifique des fraudes non detectees a ete realisee avec le script \texttt{src/analyze\_missed\_frauds.py}. L'objectif n'etait pas seulement de mesurer le nombre d'erreurs, mais de comprendre leur profil afin d'identifier les limites du modele et de proposer des contre-mesures operationnelles. Cette analyse est importante car un modele de fraude ne doit pas uniquement etre evalue sur ses succes : les faux negatifs representent les transactions frauduleuses qui passent sous le radar et qui generent donc une perte directe pour la banque.

Le script charge les probabilites du modele final retenu (fichier \texttt{tuned\_predictions.npz}), reconstruit le meme echantillon de validation stratifie, puis compare trois groupes : les fraudes detectees, les fraudes ratees et les transactions legitimes. L'analyse est menee au \textbf{seuil de decision retenu de 0,56}, afin de mesurer les faux negatifs reellement subis en exploitation. Sur l'ensemble de validation, \textbf{952 fraudes sur 4 133}, soit 23,0 \%, obtiennent un score inferieur a 0,56. A titre de sensibilite, un seuil plus permissif de 0,40 ne ramenerait ce nombre qu'a 801 fraudes ratees (19,4 \%) : l'essentiel des faux negatifs ne se joue donc pas au voisinage du seuil.

\begin{table}[H]
\centering
\caption{Synthese de l'analyse des fraudes non detectees au seuil retenu 0,56}
\begin{tabular}{lc}
\toprule
\textbf{Indicateur} & \textbf{Valeur} \\
\midrule
Fraudes totales dans l'ensemble de validation & 4 133 \\
Fraudes non detectees avec score $< 0,56$ & 952 \\
Part des fraudes non detectees & 23,0\% \\
Fraudes proches du seuil ($0,46 \leq score < 0,56$) & 97 \\
Fraudes avec score tres bas ($score < 0,10$) & 458 \\
Montant median des fraudes ratees & 97 USD \\
Montant median des fraudes detectees & 67 USD \\
Rappel : fraudes non detectees au seuil permissif 0,40 & 801 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/missed_frauds_analysis.png}
\caption{Analyse des fraudes non detectees : distribution des scores et comparaison des montants}
\label{fig:missed-frauds}
\end{figure}

Les \textbf{97 fraudes proches du seuil} constituent une zone grise interessante. Elles ne sont pas totalement invisibles pour le modele : leur score est deja eleve, mais pas assez pour etre classees comme suspectes au seuil retenu. Elles pourraient etre recuperees en abaissant le seuil, au prix d'une augmentation du nombre de fausses alertes. Cette observation confirme que le choix du seuil est un arbitrage metier entre recall et precision. Elle montre aussi que la marge de progression par simple deplacement du seuil est limitee : ces 97 cas ne representent que 10 \% des faux negatifs.

Les \textbf{458 fraudes avec un score inferieur a 0,10}, soit pres de la moitie des faux negatifs, sont plus critiques. Le modele les considere comme tres proches de transactions legitimes. Ces cas illustrent une limite naturelle du machine learning supervise : lorsque les fraudeurs imitent fortement les comportements normaux et que les variables disponibles ne contiennent pas de signal distinctif suffisant, le modele ne peut pas separer correctement fraude et legitimite. L'ajout d'un algorithme plus complexe ne suffirait pas necessairement ; il faudrait surtout enrichir les donnees.

L'analyse qualitative montre que les fraudes ratees ressemblent davantage aux transactions legitimes : elles sont associees au produit W dans 68 \% des cas (contre 36 \% pour les fraudes detectees, et 76 \% pour les transactions legitimes), a des cartes debit (63 \%), a des domaines email courants et surtout a une absence d'information device dans 71 \% des cas (contre 37 \% pour les fraudes detectees). Elles ne presentent donc pas les marqueurs classiques des fraudes plus faciles a detecter.

Un point merite une attention particuliere : les fraudes ratees portent sur des \textbf{montants plus eleves} que les fraudes detectees (mediane de 97 USD contre 67 USD, moyenne de 191 USD contre 144 USD). Le modele rate donc proportionnellement les cas les plus couteux. Cela signifie que l'estimation d'impact financier presentee plus haut, qui applique un montant moyen uniforme de 149 USD, est plutot optimiste sur la partie non detectee : la perte residuelle reelle est superieure a celle que suggere le calcul moyen. C'est un argument supplementaire en faveur des contre-mesures ci-dessous plutot qu'un simple abaissement du seuil. Cette conclusion permet de formuler une recommandation operationnelle claire : pour reduire davantage les faux negatifs, il faut completer le scoring ML par des donnees comportementales et des controles metier.

Les contre-mesures recommandees sont les suivantes :
\begin{itemize}
    \item ajouter l'historique client afin de comparer chaque transaction au comportement habituel du porteur ;
    \item integrer des signaux de geolocalisation, d'adresse IP et d'empreinte device ;
    \item declencher une verification humaine ou une authentification renforcee pour les transactions proches du seuil ;
    \item suivre separement les fraudes avec score tres bas, car elles peuvent signaler l'apparition de nouvelles strategies de fraude.
\end{itemize}

Cette analyse renforce donc le positionnement du projet : la solution finale ne doit pas etre vue comme un simple classifieur, mais comme un systeme hybride combinant modele ML, seuils metier, investigation humaine et enrichissement progressif des donnees.
...
La structure principale comprend un dossier \texttt{data} pour les donnees brutes et traitees, un dossier \texttt{src} pour les scripts Python, un dossier \texttt{models} pour les modeles sauvegardes, un dossier \texttt{reports} pour les resultats et les figures, ainsi qu'un fichier \texttt{app\_dashboard.py} pour l'application Streamlit. Cette separation permet de distinguer les artefacts de calcul des artefacts de restitution.
...
\texttt{reports/figures} & Graphiques generes pour le dashboard et le memoire. \\
...
La reproductibilite repose sur trois elements : les scripts, les donnees traitees et les rapports generes. Chaque resultat important du memoire doit pouvoir etre relie a un script ou a un fichier de sortie. Les metriques finales proviennent du script d'optimisation des hyperparametres, les KPIs metier proviennent des analyses business, et les figures proviennent du dossier \texttt{reports/figures}.

Pour une version finale, il est recommande d'ajouter un fichier \texttt{README} plus detaille, listant l'ordre d'execution exact, les dependances et les commandes de reproduction. Cette recommandation figure egalement dans les perspectives operationnelles.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/business_montants.png}
\caption{Distribution des montants et impact metier}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/business_temporel.png}
\caption{Analyse temporelle de la fraude}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/business_cartes.png}
\caption{Risque par type de carte}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/business_emails.png}
...
\chapter{Annexe I - Script d'analyse des fraudes non detectees}

Le script \texttt{src/analyze\_missed\_frauds.py} complete l'evaluation du modele final en analysant les faux negatifs. Il charge les predictions du modele final retenu, sauvegardees dans \texttt{reports/results/tuned\_predictions.npz}, reconstruit l'ensemble de validation avec le meme split stratifie, puis identifie les fraudes dont le score est inferieur au seuil de decision de 0,56. Un seuil permissif de 0,40 est egalement calcule a titre de sensibilite.

Les sorties principales du script sont :
\begin{itemize}
    \item un rapport texte sauvegarde dans \texttt{reports/results/missed\_frauds\_report.txt} ;
    \item une figure sauvegardee dans \texttt{reports/figures/missed\_frauds\_analysis.png} ;
    \item une comparaison entre fraudes ratees, fraudes detectees et transactions legitimes selon le montant, le produit, le type de carte, l'heure, l'email, le device et le taux de valeurs manquantes.
\end{itemize}

Cette annexe justifie la demarche d'analyse d'erreur et montre que les limites du modele ont ete investiguees de maniere reproductible, a partir de scripts et de fichiers de sortie verifiables.

\chapter{Annexe J - Structure recommandee pour la soutenance}
